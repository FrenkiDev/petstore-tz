...
Свойство в котором храниться информация о endpoint (путь, параметры заголовка, пути к шаблонам запросов и схем валидации ответа)
[source,java]
include::src/test/java/endpoints/User.java[lines=15..17]

Свойства для заполнения тела запроса:
[source,java]
include::src/test/java/endpoints/User.java[lines=18..27]

Методы:
[source,java]
include::src/test/java/endpoints/User.java[lines=28..]
